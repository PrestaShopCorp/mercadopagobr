...
class MercadoPagoBr extends PaymentModule {
    ...
    private function setCustomSettings($client_id, $client_secret, $country) {
        if ($country == "MLB" || $country == "MLM" || $country == "MLA" || $country == "MLC" || $country == "MCO" || $country == "MLV") {
            Configuration::updateValue ( 'MERCADOPAGO_CREDITCARD_BANNER', (Configuration::get ( 'PS_SSL_ENABLED' ) ? 'https://' : 'http://') . htmlspecialchars ( $_SERVER ['HTTP_HOST'], ENT_COMPAT, 'UTF-8' ) . __PS_BASE_URI__ . 'modules/mercadopagobr/views/img/' . $country . '/credit_card.png' );
        ...
        Configuration::updateValue ( 'MERCADOPAGO_STANDARD_BANNER', (Configuration::get ( 'PS_SSL_ENABLED' ) ? 'https://' : 'http://') . htmlspecialchars ( $_SERVER ['HTTP_HOST'], ENT_COMPAT, 'UTF-8' ) . __PS_BASE_URI__ . 'modules/mercadopagobr/views/img/' . $country . '/banner_all_methods.png' );
    }
    ...
    public function hookPayment($params) {
        ...
        $order_cart_rule->id_order = $cart->id;
        $order_cart_rule->id_cart_rule = $cart->id;
        $order_cart_rule->id_order_invoice = 1;
        $order_cart_rule->name = "teste";
        $order_cart_rule->value = 10;
        $order_cart_rule->value_tax_excl = 2;
        $order_cart_rule->add();
        /*fim teste*/
        ...
        if ($this->hasCredential ()) {
            ...
            $data = array (
                    ...
                    'discount_action_url' => $this->link->getModuleLink ( 'mercadopagobr', 'discount', array (), Configuration::get ( 'PS_SSL_ENABLED' ), null, null, false ),
